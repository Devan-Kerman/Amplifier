...
class Wikipedia(datasets.BeamBasedBuilder):
	...
	def _build_pcollection(self, pipeline, filepaths, language):
		...
		def _clean_content(inputs, language):
			...
			try:
				text, references_locs, references = _parse_and_clean_wikicode(raw_content, parser=mwparserfromhell, language=language)
			...
			# TODO section loc?
			yield id_, {"id": id_, "url": url, "title": title, "text": text, "reference_locs": references_locs, "references": references}
...
def _parse_and_clean_wikicode(raw_content, parser, language):
	...
	# TODO we don't want the citations in the raw text, so the cleaning process is correct
	# TODO maybe the raw references should be included, so the AI knows websites to look for instead of trying to memorize
	# TODO we only need the connections for wikilinks anyways
	# however we do want to keep track of the citations in a separate field, including their locations in the text
	# will probably have to replace citations with a special token or something and then replace it after to find the string locations
	...
	match_reference = re.compile(f"^(?:{media_prefixes}):", flags=re.IGNORECASE | re.UNICODE)

	def is_content_wikilink(obj):
		return bool(match_reference.match(str(obj.title)))
	...
	for section in wikicode.get_sections(flat=True, include_lead=True, include_headings=True):
		for obj in section.ifilter_wikilinks(recursive=True):
			if is_content_wikilink(obj):
				section.replace(obj, f"[Image {obj.text}]")
			elif is_category(obj):
				try_replace_obj(obj)
			else:
				full_name = f"[{obj.title}@wikilink]"
				references[full_name] = obj.title
				obj.text = obj.__strip__() + full_name
		for obj in section.ifilter_tags(matches=rm_tag, recursive=True):
			if obj.tag == 'ref':
				name = obj.get('name') if obj.has('name') else 'noname'
				group = obj.get('group') if obj.has('group') else 'nogroup'
				if name == 'noname':
					full_name = f"[unidentified{len(references)}@{group}]"
				else:
					full_name = f"[{name}@{group}]"
				if len(obj.contents) > 0:
					references[full_name] = obj.contents
				obj.contents = full_name
			else:
				# todo convert tables (?)
		...
		for obj in section.ifilter_templates():
			if obj.name =='#tag:ref':
				group = obj.get('group') if obj.has('group') else 'nogroup'
				full_name = f"[unidentified{len(references)}@{group}]"
				references[full_name] = obj.get(1)
				section.replace(obj, full_name)

		section_text.append(re.sub(re_rm_magic, "", section.strip_code().strip()))
	print(references)
	full_text = "\n\n".join(section_text)
	# TODO append non-wikilink citations (?) per-section (?)
	match_reference = re.compile(r"\[.*?@[^[\]\s]+\]", flags=re.IGNORECASE | re.UNICODE)

	references_locs: Dict[int, str] = {}
	for match in match_reference.findall(full_text):
		index = full_text.find(match)
		if match in references:
			references_locs[index] = match
			full_text = full_text.replace(match, "", 1)
	return full_text, references_locs, references
